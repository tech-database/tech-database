...
interface Uniforms {
  iResolution: UniformValue<[number, number, number]>;
  iMouse: UniformValue<[number, number]>;
  iTime: UniformValue<number>;
  uColor0: UniformValue<RGB>;
  uColor1: UniformValue<RGB>;
  uColor2: UniformValue<RGB>;
  uColor3: UniformValue<RGB>;
  uColor4: UniformValue<RGB>;
  uColor5: UniformValue<RGB>;
  uColor6: UniformValue<RGB>;
  uColor7: UniformValue<RGB>;
  uColorCount: UniformValue<number>;
  uBgColor: UniformValue<RGB>;
  uMouseColor: UniformValue<RGB>;
  uSpeed: UniformValue<number>;
  uStreakCount: UniformValue<number>;
  uStreakWidth: UniformValue<number>;
  uStreakLength: UniformValue<number>;
  uGlow: UniformValue<number>;
  uDensity: UniformValue<number>;
  uTwinkle: UniformValue<number>;
  uZoom: UniformValue<number>;
  uBgGlow: UniformValue<number>;
  uOpacity: UniformValue<number>;
  uMouseEnabled: UniformValue<number>;
  uMouseStrength: UniformValue<number>;
  uMouseRadius: UniformValue<number>;
}
...
const prepColors = (input: string[]) => {
  const base = (input.length ? input : ['#A8FFB6', '#27FF64', '#7CFF67']).slice(0, MAX_COLORS);
  const arr: RGB[] = [];
...
const Lightfall: React.FC<LightfallProps> = ({
  ...
  const serializedColors = colors.join(',');
  ...
  useEffect(() => {
    ...
    uniforms.uStreakCount.value = Math.max(1, Math.min(16, Math.round(streakCount)));
    uniforms.uStreakWidth.value = streakWidth;
    uniforms.uStreakLength.value = streakLength;
    uniforms.uGlow.value = glow;
    uniforms.uDensity.value = density;
    uniforms.uTwinkle.value = twinkle;
    uniforms.uZoom.value = zoom;
    uniforms.uBgGlow.value = backgroundGlow;
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (propsRef.current.mouseDampening > 0) {
        ...
        lastTime = time;
        const tau = Math.max(1e-4, propsRef.current.mouseDampening);
  ...
  }, [backgroundColor, dpr, mouseInteraction]);
